...
char cut[20];
strcpy(cut, "-0.8<mc_xF&&mc_xF<-0.7");   // First cut applied on variable of focus, change for different variable

float start = -0.7;      // Larger value of first cut made on target variable
float end = 0.9;        // Number one increment greater than the larger value of the final cut (getting around the for-loop being not inclusive)
...
float err3;
	...
	outputfile = fopen("Means-Xaxis-XF.txt" , "wb");  // Text file to print to
	...
	for (float j = start; j < end; j += increment) {  // Loop to run through the target variable cuts
		...
		//fprintf(outputfile, "%s \t", cut);
        	...
        	TH1F*h1 = new TH1F("h1","h1",20, -1.0, 1.0);  // Histogram for mean of generated pT
        	outTree->Draw("xF>>h1", VARcut);
		mean1 = h1->GetMean();
		err1 = h1->GetStdDev();
